...
def produce_projectives(U,  V):
 
 U_trans=copy.copy(U)
 U_trans.transpose()
 U_trans.setLabel([1,-1,-2,-200]) 
 
 V_trans=copy.copy(V)
 V_trans.transpose()
 V_trans.setLabel([-1,-2,-200,2])
 
 A=U_trans*V_trans
 A.permute([1,2],1)
 Ub, sb, Vb=TruncateU.svd_parity(A) 
# print sb
 sb=TruncateU.inverse(sb)
 sb=TruncateU.Sqrt(sb)
 U.setLabel([1,2,200,3])
 Ub.setLabel([3,4])
 sb.setLabel([4,5])
 U1x=U*Ub*sb
 U1x.permute([1,2,200,5],3)
 U1x_trans=copy.copy(U1x)
 U1x_trans.transpose()
 U1x_trans.setLabel([6,3,4,100])


 
 V.transpose()
 V.setLabel([1,2,200,3])
 Vb.transpose()
 Vb.setLabel([3,4])
 sb.transpose()
 sb.setLabel([4,6])
 V1x_trans=V*Vb*sb
 V1x_trans.permute([1,2,200,6],3)
 V1x=copy.copy(V1x_trans)
 V1x.transpose()
 V1x.setLabel([3,4,100,5])
...
# print A,A[0]
# 
# A=V1x_trans*U1x_trans
# A.permute([1,2,200,3,4,100], 3)
# #print A
######################################################
 #U1x=copy.copy(V1x_trans)


 U1x_trans=copy.copy(V1x)
 U1x=copy.copy(U1x)

# U1x_trans=copy.copy(U1x_trans)
# U1x=copy.copy(V1x_trans)
...
# B=V1x_trans*U1x_trans
# B.permute([6,5],1) 
# U1, s1, V1=TruncateU.svd_parity(B) 
# s1=TruncateU.Sqrt(s1)
# U1.setLabel([1,2])
# s1.setLabel([2,3])
# V1.setLabel([3,4])
# U1=U1*s1
...
# 
# U1x=V1x_trans*U1
# U1x_trans=V1*U1x_trans
# 
# U1x.permute([4,2,200,3],3)
# U1x_trans.permute([2,1,3,200],1)
...
def  add_left1(c1,c2,c3,c4,Ta1,Ta2,Ta3,Ta4,Tb1,Tb2,Tb3,Tb4,a,b,c,d,chi,D):
 ...
 U1x, U1x_trans=produce_projectives(U,  V)
 ...
 U,  V,  s = TruncateU.setTruncation(theta, chi_dim)
 U2x, U2x_trans=produce_projectives(U,  V)
 ...
 U,  V,  s = TruncateU.setTruncation(theta, chi_dim)
 U3x, U3x_trans=produce_projectives(U,  V)
 ...
 U4x, U4x_trans=produce_projectives(U,  V)
