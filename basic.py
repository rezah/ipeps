...
def produce_GammaLanda(q_D, q_phys):
 ...
 return Gamma_a,Gamma_b,Landa_1,Landa_2,Landa_3,Landa_4
...
def make_Hamz2(H_M):
 M_0=uni10.Matrix(32,32)
 M_1=uni10.Matrix(32,32)
 dic_z2={}
 sum1=0
 sum2=0
 dic_1=uni10.Matrix(64,1);
 dic_2=uni10.Matrix(64,1);
  ...
  for i2 in xrange(2):
   for i3 in xrange(2):
    for j1 in xrange(2):
     for j2 in xrange(2):
      for j3 in xrange(2):
         if ((i1+i2+i3+j1+j2+j3)%2)==0:
            dic_1[32*i1+16*i2+8*i3+4*j1+2*j2+j3]=sum1
            sum1+=1
         if ((i1+i2+i3+j1+j2+j3)%2)==1:  
            dic_2[32*i1+16*i2+8*i3+4*j1+2*j2+j3]=sum2
            sum2+=1
        ...
        for m2 in xrange(2):
         for m3 in xrange(2):
          for n1 in xrange(2):
           for n2 in xrange(2):
            for n3 in xrange(2):
              if ((i1+i2+i3+j1+j2+j3)%2)==0 and ((m1+m2+m3+n1+n2+n3)%2)==0: 
                b=int(dic_1[32*i1+16*i2+8*i3+4*j1+2*j2+j3])
                bp=int(dic_1[32*m1+16*m2+8*m3+4*n1+2*n2+n3])
                M_0[32*bp+b]=H_M[2048*m1+1024*m2+512*m3+256*n1+128*n2+64*n3+32*i1+16*i2+8*i3+4*j1+2*j2+j3]
              if ((i1+i2+i3+j1+j2+j3)%2)==1 and ((m1+m2+m3+n1+n2+n3)%2)==1: 
                b=int(dic_2[32*i1+16*i2+8*i3+4*j1+2*j2+j3])
                bp=int(dic_2[32*m1+16*m2+8*m3+4*n1+2*n2+n3])
                M_1[32*bp+b]=H_M[2048*m1+1024*m2+512*m3+256*n1+128*n2+64*n3+32*i1+16*i2+8*i3+4*j1+2*j2+j3]
 return M_0, M_1
...
#    bdo = uni10.Bond(uni10.BD_OUT, d_phys)
#    H = uni10.UniTensor([bdi, bdi, bdo, bdo], "Heisenberg")
#    H_uni=Heisenberg0(h, J1)
#    H_M=H_uni.getBlock()

#    #print "Symmetric", H_M

#    M_0, M_1=make_Hamz2(H_M)
#    H.putBlock(blk_qnums[0],M_0)
#    H.putBlock(blk_qnums[1],M_1)
...
def E_total_conv(a_u,b_u,a,b,Env,Env1,D,h,d_phys,chi,Corner_method,Model):
...
##############################################################################
 ...
 print E_ab 
 E_ba=Energy_h(b_u,a_u,b,a,d,c,Env1,D,h,d_phys,chi,Corner_method,Model)
 print E_ba

 E_ca=Energy_v(b_u,a_u,a,b,c,d,Env,D,h,d_phys,chi,Corner_method,Model)
 print E_ca
 E_ac=Energy_v(a_u,b_u,b,a,d,c,Env1,D,h,d_phys,chi,Corner_method,Model)
 print E_ac 
###############################################################################
...
# return (E_ab+E_ba)/2.0
 return ((E_ab+E_ba+E_ca+E_ac) / 2.00)#+((E_val1+E_val2+E_val3+E_val4) / 4.00) + ((E_val5+E_val6+E_val7+E_val8) / 4.00)
...
def E_total(a_u,b_u,a,b,Env,Env1,D,h,d_phys,chi,Corner_method,Model):
############################################################################
# E_val1=Energy_cb(c_u,b_u,a,b,c,d,Env,D,h,d_phys,chi,Corner_method,Model)
# #print "E_val1", E_val1
...
def Store_itebd(Gamma_a,Gamma_b,Landa_1,Landa_2,Landa_3,Landa_4):
...
def Reload_Full_previous(a_u, b_u):
...
########################################
 Delta_N=((5.0/100),N_iterF)
 List_delN.append(Delta_N)

 Delta_N=((2.0/100),N_iterF)
 List_delN.append(Delta_N)
